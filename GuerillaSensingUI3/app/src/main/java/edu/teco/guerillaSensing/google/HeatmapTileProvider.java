...
package edu.teco.guerillaSensing.google;
...
public class HeatmapTileProvider implements TileProvider {
     ...
     */
    public static final double DEFAULT_OPACITY = 0.675;
    ...
    static Object[] convolve(float[][] grid, int[][] count, float[] kernel) {
        ...
        for (x = 0; x < dim; x++) {
            for (y = 0; y < dim; y++) {
                if (additionsY[x][y] != 0 && outputGrid[x][y] != 0) {
    ...
    static int adjustAlpha(int color, double factor) {
        ...
        return Color.argb((int) (alpha * DEFAULT_OPACITY) , red, green, blue);
    }
